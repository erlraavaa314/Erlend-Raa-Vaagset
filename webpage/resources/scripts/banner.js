...
function drawLeftBanner() {
    ...
    const ctx = leftCanvas.getContext('2d'); // Get the 2D rendering context

    // Set canvas size
    const width = leftCanvas.clientWidth;
    const height = leftCanvas.clientHeight;
    leftCanvas.width = width;
    leftCanvas.height = height;

    // Draw using penrosetiling.js logic
    draw(ctx, width, height); // Assuming draw() from penrosetiling.js draws on the specified canvas
}

// Right Banner
function drawRightBanner() {
    const rightCanvas = document.getElementById('rightCanvas');
    const ctx = rightCanvas.getContext('2d'); // Get the 2D rendering context
