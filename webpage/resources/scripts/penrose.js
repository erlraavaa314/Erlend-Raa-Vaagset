...
function setup() {
    // Create an SVG canvas instead of a regular canvas
    createCanvas(1, 1, SVG);
    ds = new PenroseLSystem();
    ds.simulate(5); // Adjust the number of generations as needed
    noLoop(); // Ensure draw function is called only once
}

function draw() {
    // The draw function is called only once due to noLoop()
    save("leftBanner.svg"); // Save the left banner as an SVG file
    createRightBanner(); // Create the right banner and save it as SVG
    noLoop(); // Prevent additional draw calls
}

function createRightBanner() {
    // Create a new instance of PenroseLSystem for the right banner
    ds.reset();
    ds.simulate(5); // Adjust the number of generations as needed
    save("rightBanner.svg"); // Save the right banner as an SVG file
}
